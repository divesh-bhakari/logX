...
try:
    db = mysql.connector.connect(
        host="localhost",
        user="root",
        password="Divesh@123",  # <-- replace this with your actual password
        database="logx_db"
    )
    cursor = db.cursor()
...
def upload_logs():
    ...
    return jsonify({"message": "Log analyzed successfully!", "result": result})
...
def analyze_system_logs(content):
    # Look for boot, shutdown, kernel messages
...
def analyze_server_logs(content):
    # Look for HTTP methods, endpoints
...
def analyze_custom_logs(content):
    # Just extract patterns like key=value
